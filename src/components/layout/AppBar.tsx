  ...
  return (
    <>
      ...
      >
        ...
        <HStack gap={0} ml={1}>
          <ProjectSwitcher open={switcherOpen} onClose={() => setSwitcherOpen(false)} onCreateNew={handleCreateNew}>
            <Button size="sm" variant="ghost" onClick={() => setSwitcherOpen(!switcherOpen)} px={2}>
          ...
          <Tooltip content="项目设置">
            <IconButton aria-label="Project settings" size="xs" variant="ghost" onClick={handleOpenSettings}>
              <FiSettings />
            </IconButton>
          </Tooltip>
        </HStack>
        ...
        <HStack gap={1}>
          ...
          <Tooltip content={colorMode === "dark" ? "切换浅色模式" : "切换深色模式"}>
            <IconButton aria-label="Toggle color mode" size="sm" variant="ghost" onClick={toggleColorMode}>
              {colorMode === "dark" ? <FiSun /> : <FiMoon />}
            </IconButton>
          </Tooltip>
          <ExportMenu />
          <Tooltip content="保存">
            <IconButton aria-label="Save" size="sm" variant="ghost" onClick={handleSave}>
              <FiSave />
            </IconButton>
          </Tooltip>
        </HStack>
      ...
      <ProjectSettingsDialog open={settingsOpen} onClose={() => setSettingsOpen(false)} mode={settingsMode} />
    </>
  );
}
