...
function importProjectJSON(json: string): {
  ...
  return {
    nodes: data.nodes as ProjectNode[],
    globalThemeId: data.globalThemeId,
    name: data.name,
...
export async function saveProjectToFile(
  ...
  const handle = await (
    window as unknown as { showSaveFilePicker: (opts: object) => Promise<FileSystemFileHandle> }
  ).showSaveFilePicker({
...
export async function loadProjectFromFile(): Promise<{
  ...
  const [handle] = await (
    window as unknown as { showOpenFilePicker: (opts: object) => Promise<FileSystemFileHandle[]> }
  ).showOpenFilePicker({
    types: [{ description: "JSON", accept: { "application/json": [".json"] } }],
  });
